...
def straighten(image):
    ...
    dialted2 = dilated.copy()
    ctrs,hier = cv2.findContours(dialted2,cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_SIMPLE)
...
def extractWords(image, lines):
    ...
    for i, line in enumerate(lines):
        ...
        line[0]=int(line[0])
        line[1]=int(line[1])
        extract = thresh[line[0]:int(line[1]), 0:width] # y1:y2, x1:x2
...
# This function helps get Average Letter Size
...
def calculatePenPressure(image):
    ...
    #This costs the maximum time and processing in the entire process!
    #is_grey_scale(imageGS)
    #     print(isGray)
    ...
    # add up all the non-zero pixel values in the image and divide by the number of them to 
    # find the average pixel value in the whole image
    total_intensity = 0
    pixel_count = 0
    average_intensity =0
    for x in range(h):
