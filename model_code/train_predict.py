...
if os.path.isfile("label_list.txt"):
    ...
    dump(clf1, 'knowme_EmotionalSt.joblib')
    # clf1.save("knowme_EmotionalSt.h5")
    print("Emotional Stability Score: ",accuracy_score(clf1.predict(X_test), y_test))
    
    X_train, X_test, y_train, y_test = train_test_split(X_t2, y_t2, test_size = .30, random_state=16)
    clf2 = SVC(kernel='rbf')
    clf2.fit(X_train, y_train)
    dump(clf2, 'knowme_MentalE_WlPower.joblib')
    ...
    dump(clf3, 'knowme_Modesty.joblib')
    ...
    dump(clf5, 'lackOfDiscipline.joblib')
    ...
    dump(clf6, 'PoorConcentration.joblib')
    ...
    dump(clf8, 'SocialIsolation.joblib')
